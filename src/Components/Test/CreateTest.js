...
export const CreateTest = () => {
  ...
  const [enterTestName , setEnterTetName]= useState("")
  useEffect(() => {
  ...
  const handleSaveQuestions = async () => {
    ...
    const testid = await handleSaveQuestionPapernameAndGetId()
    for (let q of questions) {
      ...
      const payload = {
        ...
        standard: standard,
        subject: subject,
        test_id: testid, // Replace with actual test_id if needed
      };
  ...
  const handleSaveQuestionPapernameAndGetId = async () => {
      ...
      try {
        const result = await apiRequest({
          endpoint: "createQuestionPapername/createQuestionPapername.php",
          method: "POST",
          data: payload,
        });
  ...
  const handleGetQuestions = async () => {
    ...
    try {
      ...
      const result = await apiRequest({
        endpoint:
          userData.role === "admin"
            ? "questionbank/getallquestion.php"
            : "questionbank/getSchoolsubjectteacherSpecificQuestions.php",
        method: "POST",
        ...
        <main style={mainContentStyle}>
          ...
          <div>
           {filteredQuestions.length >0 && <input
              ...
              value={enterTestName}
              onChange={(e)=>{setEnterTetName(e.target.value)}}
            />}
            <h3
              style={{
                fontSize: "18px",
                      ...
                      {[1, 2, 3, 4].map((optionNum) => (
                        ...
                        >
                          <input
                            ...
                            checked={
                              question.correct_answer === optionNum.toString()
                            }
                            onChange={() =>
                              updateQuestion(
                                question.id,
            ...
            {questions.length === 0 && (
              ...
              >
                No questions available. Click "Get Question Bank" to load
                questions or "+" to add new ones.
              </div>
            )}
          </div>
          ...
          >
            ...
            <button style={buttonStyle} on onClick={handleSaveQuestions}>
              Save
            </button>
          </div>
        </main>
